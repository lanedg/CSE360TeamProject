...
public class ValidateUserNameInput {
	...
	/**********
	 * Simple method use to check the length of the username only.
	 * 
	 * @param input		The input string for the Finite State Machine
	 * @return			An output string that is empty if every things is okay or it is a String
	 * 						with a helpful description of the error
	 */
	public static String checkUserLength(String input) {
		int usernameSize = input.length();
		if (usernameSize < 4 || usernameSize > 32) {
			//user name too long or too short
			return "Username must be 4 - 32 characters long.";
		}
		//user name has no issue
		return "";
	}
}
